...
int main() {
  ...
  int res =
      bind(vsock_socket, (struct sockaddr *)&vsock_addr, sizeof(vsock_addr));
  if (res < 0) {
    printf("bind error: %d, %s\n", res, strerror(errno));
    return -1;
  }

  res = listen(vsock_socket, 0);
  if (res < 0) {
  ...
  while (1) {
    int conn = accept(vsock_socket, 0, 0);
    if (conn < 0) {
    ...
    while (1) {
      ssize_t res = recv(conn, buf, 65536, 0);

...
func main() {
	...
	exist_amd64 := instances.CheckAMIFromNameTag("marlin/oyster/worker-salmon-amd64-"+date, profile, region)
	exist_arm64 := instances.CheckAMIFromNameTag("marlin/oyster/worker-salmon-arm64-"+date, profile, region)
...
func create_ami(keyPairName string, keyStoreLocation string, profile string, region string, arch string, date string) {
	...
	amiName := "marlin/oyster/worker-salmon-" + arch + "-" + date
